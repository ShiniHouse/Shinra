...
| :--- | :--- | :--- | :--- |
| SEC-01 | Critico | Un solo endpoint su trentanove verifica l'autenticazione. `POST /api/modes/{nome}/activate` esegue una routine domotica senza credenziali. | Aperto |
| SEC-02 | Critico | `/api/alexa` non verifica la firma Amazon ne' l'`applicationId`. Se esposto su Internet, accetta comandi da chiunque. | Aperto |
...
| SEC-05 | Alto | `config/config.yaml` e' tracciato da git e riceve il token Home Assistant al salvataggio dalle impostazioni. | Aperto |
...
1. **Non esporre `/api/alexa` su Internet.** E' l'unico punto raggiungibile
   dall'esterno e non ha alcuna verifica.
