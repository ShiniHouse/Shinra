...
async function loadRuoli() {
    ...
    container.innerHTML = _html`${Stato.ruoli.map((r) => {
        ...
        <div class="p-4 rounded-2xl bg-slate-900/60 border border-slate-800 space-y-3 hover:border-slate-700 transition">
            <div class="flex justify-between items-start gap-3">
                <div>
                    <h4 class="font-bold text-xs text-slate-100 flex items-center gap-2">
                        ${r.nome}
                        ${r.predefinito ? _grezzo('<span class="px-1.5 py-0.5 rounded bg-indigo-950/60 border border-indigo-900/50 text-[9px] text-indigo-300 font-mono">predefinito</span>') : ''}
                    </h4>
            ...
            <div class="flex flex-wrap gap-1.5">${etichette.length ? etichette : _grezzo('<span class="text-[10px] text-slate-500 italic">Nessun permesso: puo&#39; solo chiedere.</span>')}</div>
...
function apriModaleRuolo(idRuolo) {
    ...
    const caselle = permessiCatalogo.map((p) => {
        ...
        <label class="flex items-start gap-2.5 p-2.5 rounded-xl bg-slate-950 border ${rischioso ? 'border-rose-900/50' : 'border-slate-800'} hover:border-slate-700 cursor-pointer transition">
            <input type="checkbox" class="permesso-casella mt-0.5 accent-indigo-500" value="${p.id}" ${scelti.includes(p.id) ? _grezzo('checked') : ''}>
            <span class="leading-tight">
                <span class="block text-[11px] font-semibold ${rischioso ? 'text-rose-300' : 'text-slate-200'}">${p.descrizione}</span>
                <span class="block text-[10px] text-slate-500 font-mono">${p.id}</span>
            </span>
        </label>`;
    });
    ...
    showModal(
        ...
        <div class="space-y-4 pt-3">
            <div>
                ...
                <input type="text" id="ruolo-nome" value="${ruolo ? ruolo.nome : ''}" placeholder="es. Collaboratrice domestica, Nonno, Ospite fine settimana" class="w-full bg-slate-950 border border-slate-700 rounded-xl px-3 py-2 text-xs text-slate-100 placeholder-slate-600 focus:outline-none focus:border-indigo-500">
            </div>
            <div>
                <label class="text-[11px] font-semibold text-slate-300 block mb-1">Descrizione (opzionale):</label>
                <input type="text" id="ruolo-descrizione" value="${ruolo ? ruolo.descrizione : ''}" placeholder="A cosa serve questo ruolo" class="w-full bg-slate-950 border border-slate-700 rounded-xl px-3 py-2 text-xs text-slate-100 placeholder-slate-600 focus:outline-none focus:border-indigo-500">
            </div>
            <div>
                <label class="text-[11px] font-semibold text-slate-300 block mb-1.5">Cosa puo' fare:</label>
                <div class="grid grid-cols-1 gap-1.5 max-h-72 overflow-y-auto pr-1">${caselle.length ? caselle : _grezzo('<p class="text-[11px] text-slate-500">Catalogo dei permessi non disponibile.</p>')}</div>
            </div>
        </div>
...
async function loadDispositivi() {
    ...
    try {
        ...
        container.innerHTML = _html`<div class="p-4 rounded-2xl bg-slate-900/60 border border-slate-800 text-xs text-rose-300">Impossibile leggere i dispositivi fidati.</div>`;
        return;
    }
            ...
            <div class="flex items-center gap-3 min-w-0">
                ...
                <div class="min-w-0">
                    <h4 class="font-bold text-xs text-slate-100 flex items-center gap-1.5 truncate">
                        ${d.nome}
                        ${d.questo ? _grezzo('<span class="px-1.5 py-0.5 rounded bg-indigo-950/60 border border-indigo-900/50 text-[9px] text-indigo-300 font-semibold shrink-0">questo dispositivo</span>') : ''}
                    </h4>
                    <p class="text-[10px] text-slate-500 truncate">
                        ${nomeDi(d.user_id)} · ultimo accesso ${_quando(d.ultimo_uso)}${d.ultimo_indirizzo ? ` · ${d.ultimo_indirizzo}` : ''}
                    </p>
                </div>
            </div>
