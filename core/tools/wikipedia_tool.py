...
async def search_wikipedia(query: str, language: str = "it") -> Dict[str, Any]:
    ...
    clean_query = re.sub(
        r"^(cosa significa|chi era|chi è|definizione di|cos'è|spiegami|il termine|la parola|che cos'è)\s+",
        "",
        query.strip(),
        flags=re.IGNORECASE,
    ).strip(" ?.,\"'")
    ...
    try:
        ...
        async with httpx.AsyncClient(timeout=8.0, follow_redirects=True) as client:
            ...
            if search_res.status_code == 200:
                ...
                if results:
                    ...
                    return {"success": True, "termine": title, "estratto": snippet}

            return {
                "success": False,
                "message": f"Nessuna informazione enciclopedica trovata per '{clean_query}'.",
            }
        ...
        return {
            "success": False,
            "error": str(e),
            "message": f"Impossibile consultare Wikipedia per '{clean_query}'.",
        }
